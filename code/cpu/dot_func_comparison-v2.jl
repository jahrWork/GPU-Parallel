# import Pkg
# Pkg.activate(".")
# Pkg.add(["CPUTime", "Plots", "LinearAlgebra", "MKL"])
using CPUTime
using Plots
...
# Create the plot using Plots.jl with y-axis limited to 10 GFLOPS
# plot(x, y1, label="matrix_multiplication", linewidth=2, ylim=(0, 10))
plot(x, y1, label="matrix_multiplication", linewidth=2)
plot!(x, y2, label="my_matrix_multiplication", linewidth=2, linestyle=:dash)
plot!(x, y3, label="my_efficient_matrix_multiplication", linewidth=2, linestyle=:dot)
plot!(x, y4_vector, label="Theoretical GFLOPS", linewidth=2, linestyle=:dashdot, color=:red)
