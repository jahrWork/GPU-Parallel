...
function scalar_matrix_multiplication(scalar, N, dtype=Float32)
  A = rand(dtype, N, N)
...
#==== PRODUCTO ESCALAR CON DIFERENTES N=====#

scalar = 2.0
N = 20000

time_scalar_A = scalar_matrix_multiplication(scalar, N, Float32)
GFLOPS_scalar_A = 1e-9 / (time_scalar_A/(N^2))

println("GFLOPS_scalar_A = ", GFLOPS_scalar_A)
display(plot!(N, GFLOPS_scalar_A, label="scalar_A", legend=:topleft))
