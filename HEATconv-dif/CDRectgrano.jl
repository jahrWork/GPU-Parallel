...
# Stability check
dt_max = 0.5 * (dx^2 * dy^2) / (alpha * (dx^2 + dy^2))
...
rect_x = Int(floor(nx/4)):Int(floor(3*nx/4))
rect_y = Int(floor(ny/4)):Int(floor(3*ny/4))
...
for step in 1:nsteps
   ...
   global T = update_temperature!(T, alpha, dx, dy, dt)
...
gif(anim, "heat_transfer_simulation.gif", fps=10)
