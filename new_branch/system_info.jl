...
function cpu_info(return_max_gflops=false)
    ...
    if return_max_gflops
        if AVX_value > 0
            # Sigo pensado que esto es incorrecto, AVX_value en AVX2 es 8, pero sobre dos registros de 256 bits, fala otro factor de 2 por lo tanto.
            # Se puede ver con nuestras pruebas (conseguimos casi 700GFLOPS en mi Ryzen 5 5600X con AVX2, marcando el teórico 432GFLOPS)
            Theoretical_time = 1e9 / (cpu_frequency * 1e9 * AVX_value * 2 * num_cores) 
