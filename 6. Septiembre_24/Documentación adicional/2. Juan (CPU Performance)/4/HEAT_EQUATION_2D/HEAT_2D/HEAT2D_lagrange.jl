...
# Función para crear la matriz del operador laplaciano
function create_laplacian_matrix(nx, ny, dx, dy)
    ...
    for j in 1:ny
        for i in 1:nx
            index = (j-1)*nx + i
            if i > 1
                L[index, index-1] = 1 / dx^2
            end
            if i < nx
                L[index, index+1] = 1 / dx^2
            end
            if j > 1
                L[index, index-nx] = 1 / dy^2
            end
            if j < ny
                L[index, index+nx] = 1 / dy^2
            end
            L[index, index] = -2 / dx^2 - 2 / dy^2
        end
    end
    return L
end
...
animation = @animate for i in 1:intervalo_animacion:length(temperaturas)
    ...
    surface(
        ...
        c=:inferno, xlims=(0, Lx), ylims=(0, Ly), zlims=(z_min, z_max)
    )
end # Este `end` cierra el bloque `@animate`
